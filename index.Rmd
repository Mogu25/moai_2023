...
dat <- read_csv('./Menter_2023_2.csv') %>%
  as_tibble() %>% 
  #filter(.$'相談相手役をお引き受けいただけますか？' == "可") %>% 
  rename(
    #"Schedule" = "以下の大会開催期間のうち、モアイ実施が可能な日程を選択ください。なお時間帯はお昼ある#いは夕方以降など公式プログラムの自由時間にあたる期間を設定ください。",
    "Name" = "名前",
    "Keys" = "キーワード",
    "HP" = "HP",
    "Message" = "メッセージ"
  ) %>% 
  select(Name, Keys, Message, HP, Photo_name) %>% 
  arrange(Photo_name) %>% 
  as_tibble()
